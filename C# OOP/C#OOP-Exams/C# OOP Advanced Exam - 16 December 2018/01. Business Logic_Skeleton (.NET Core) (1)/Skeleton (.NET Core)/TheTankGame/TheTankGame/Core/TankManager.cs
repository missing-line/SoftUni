namespace TheTankGame.Core
{
    ...
    public class TankManager : IManager
    {
        ...
        public TankManager()
        {
            this.battleOperator = new TankBattleOperator();//todo
        ...
        public string AddVehicle(IList<string> arguments)
        {
            ...
            if (!this.vehicles.ContainsKey(vehicle.Model))
            {
                this.vehicles.Add(vehicle.Model,null);
            }
            this.vehicles[vehicle.Model] = vehicle;
        ...
        public string AddPart(IList<string> arguments)
        {
            ...
            if (!this.parts.ContainsKey(part.Model))
            {
                this.parts.Add(part.Model, null);
            }
            this.parts[part.Model] = part;



            if (part is IAttackModifyingPart)
            {
                this.vehicles[vehicleModel].AddArsenalPart((IAttackModifyingPart)part);
            }
            else if (part is IDefenseModifyingPart)
            {
                this.vehicles[vehicleModel].AddShellPart((IDefenseModifyingPart)part);
            }
            else if (part is IHitPointsModifyingPart)
            {
                this.vehicles[vehicleModel].AddEndurancePart((IHitPointsModifyingPart)part);
            }
