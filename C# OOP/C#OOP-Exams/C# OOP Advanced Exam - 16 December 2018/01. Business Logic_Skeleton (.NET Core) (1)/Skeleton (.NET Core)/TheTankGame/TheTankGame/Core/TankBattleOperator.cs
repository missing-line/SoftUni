    ...
    public class TankBattleOperator : IBattleOperator
    {
        public string Battle(IVehicle attacker, IVehicle target)
        {
            ...
            while (!isSomeoneDead)
            {
                if (isAttackerTurn)
                {
                    targetHitPoints -= (long)Math
                        .Max(0, Math.Ceiling(attackerAttack - (targetDefense + (targetWeight / 2))));
                    isAttackerTurn = false;
                }
                else
                {
                    attackerHitPoints -= (long)Math
                        .Max(0, Math.Ceiling(targetAttack - (attackerDefense + (attackerWeight / 2))));//todo
                    isAttackerTurn = true;
                }
