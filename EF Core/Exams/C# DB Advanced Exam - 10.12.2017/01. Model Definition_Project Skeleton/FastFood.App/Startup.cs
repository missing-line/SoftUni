namespace FastFood.App
{
    ...
    public class Startup
    {
        ...
        private static void ImportEntities(FastFoodDbContext context, string baseDir = @"C:\Users\swade\Desktop\C# DB Advanced Exam - 10.12.2017\01. Model Definition_Project Skeleton\Datasets\")
        {
            ...
            //PrintAndExportEntityToFile(employees, baseDir + exportDir + "Employees.txt");

            var items = DataProcessor.Deserializer.ImportItems(context, File.ReadAllText(baseDir + "items.json"));
           // PrintAndExportEntityToFile(items, baseDir + exportDir + "Items.txt");

            var orders = DataProcessor.Deserializer.ImportOrders(context, File.ReadAllText(baseDir + "orders.xml"));
            //PrintAndExportEntityToFile(orders, baseDir + exportDir + "Orders.txt");
        }
