...
public class EngineImpl implements Engine {
    ...
    private String manufacture(String[] data) {
        var result = this.controller.manufacture(data[0], data[1], Integer.parseInt(data[2]),  Integer.parseInt(data[3]), Integer.parseInt(data[4]));
        return result;
    }

    private String repair(String[] data) {
        var result = this.controller.repair(data[0], Integer.parseInt(data[1]));
        return result;
    }

    private String work(String[] data) {
        var result = this.controller.work(data[0], Integer.parseInt(data[1]));
        return result;
    }

    private String charge(String[] data) {
        var result = this.controller.charge(data[0], Integer.parseInt(data[1]));
        return result;
    }

    private String sell(String[] data) {
        var result = this.controller.sell(data[0], data[1]);
        return result;
    }

    private String history(String[] data) {
        var result = this.controller.history(data[0]);
        return result;
    }
}
